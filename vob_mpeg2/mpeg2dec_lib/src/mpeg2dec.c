...
static void ps_loop (void)
{
	...
	fseek(in_file, 45 * 1024 * 1024, SEEK_SET );
...
int mpeg2dec_lib_get_info(char *s_filename, VOBINFO *pr_vobinfo)
{
	...
	int i_tot_width = pr_vobinfo->i_width;
	int i_tot_height = pr_vobinfo->i_height;
	int i = 720;
	int j = 0;
	...
	if (in_file)
	{
		...
		//sprintf(out_tmp_file, "%s%s", s_tmp_folder, s_cropfile);
